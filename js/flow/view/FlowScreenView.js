...
import { Node, HBox } from '../../../scenery/js/imports.js';
import { PushButton } from '../../../sun/js/imports.js';
...
class FlowScreenView extends ScreenView {
  ...
  constructor( flowModel ) {
    ...
    const mainContainer = new Rectangle(0, 0, 800, 600, {
      fill: new Color('#F0F8FF'), // Светло-голубой фон
      stroke: '#000000',
      lineWidth: 2
    });

    // Создаем заголовок
    const title = new Text(FluidPressureAndFlowStrings.FLUID_PRESSURE_AND_FLOW.flow.flowScreenTitle, {
      font: 'bold 24px Arial',
      fill: '#000000'
    });

    // Создаем панель инструментов
    const toolsPanel = new FlowToolsControlPanel(flowModel);

    // Создаем визуализацию трубы
    const pipeNode = new PipeNode(flowModel.pipe);

    // Создаем измеритель потока
    const fluxMeter = new FluxMeterNode(flowModel.fluxMeter);

    // Создаем кнопки управления
    const playButton = new PushButton({
      content: new Text(FluidPressureAndFlowStrings.FLUID_PRESSURE_AND_FLOW.flow.play, {
        font: '16px Arial',
        fill: '#000000'
      }),
      baseColor: '#4CAF50',
      listener: () => flowModel.startSimulation()
    });

    const pauseButton = new PushButton({
      content: new Text(FluidPressureAndFlowStrings.FLUID_PRESSURE_AND_FLOW.flow.pause, {
        font: '16px Arial',
        fill: '#000000'
      }),
      baseColor: '#FFC107',
      listener: () => flowModel.pauseSimulation()
    });

    const resetButton = new PushButton({
      content: new Text(FluidPressureAndFlowStrings.FLUID_PRESSURE_AND_FLOW.flow.reset, {
        font: '16px Arial',
        fill: '#000000'
      }),
      baseColor: '#F44336',
      listener: () => flowModel.reset()
    });

    // Создаем панель кнопок
    const buttonPanel = new HBox({
      children: [playButton, pauseButton, resetButton],
      spacing: 10,
      align: 'center'
    });

    // Создаем основной макет
    const mainLayout = new VBox({
      children: [
        title,
        toolsPanel,
        pipeNode,
    ...
    // Центрируем макет
    mainLayout.center = mainContainer.center;
